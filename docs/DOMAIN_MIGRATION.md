# Domain Migration Runbook — debatekit.ai

This runbook walks the operator through registering `debatekit.ai` (if not already registered), moving DNS authority from GoDaddy to Cloudflare (under Soheil's Cloudflare account), and wiring up the subdomains the DebateKit Workers expect.

**Scope:** primarily `debatekit.ai`. The same procedure applies to any other deadpixel domains currently parked at GoDaddy that should also move to Cloudflare — see [Section 10](#10-other-domains).
...
- [ ] Decide: is `debatekit.ai` already registered?
  - Check via WHOIS: https://www.whois.com/whois/debatekit.ai
...
## 2. Register debatekit.ai (if not already)
...
1. Open https://www.godaddy.com/domainsearch/find?domainToCheck=debatekit.ai
2. Add `debatekit.ai` to cart. Set registration term to 2 years.
...
2. Enter `debatekit.ai` and click **Continue**.
...
1. Open https://dcc.godaddy.com/manage/dns?domainName=debatekit.ai (signed in as `firstexhotic@gmail.com`).
...
- Cloudflare will email Soheil's account email (`ava@deadpixel.ai` if logged in via that SSO) when activation completes. Zone status will flip to **Active** in https://dash.cloudflare.com/67bc7b518b92a0c406ac9b8526ddbb6d/debatekit.ai
- Optional local check (operator runs):

  ```bash
  dig +short NS debatekit.ai @8.8.8.8
  ```
...
```jsonc
"routes": [
  { "pattern": "api.debatekit.ai", "custom_domain": true }
]
```
...
Email for `debatekit.ai` is handled in a separate runbook so receiving (Cloudflare Email Routing) and sending (AWS SES) can evolve independently of the DNS migration.
...
- `noreply@debatekit.ai` — transactional outbound (SES)
- `support@debatekit.ai` — inbound, routed to a human inbox via Email Routing
- `hello@mail.debatekit.ai` — inbound on the `mail.` subdomain (separate MX on the `mail` subdomain)
...
In the Cloudflare dashboard for the `debatekit.ai` zone:
...
- [ ] `dig +short NS debatekit.ai @8.8.8.8` returns two `*.ns.cloudflare.com` entries
- [ ] Cloudflare zone status shows **Active** (https://dash.cloudflare.com/67bc7b518b92a0c406ac9b8526ddbb6d/debatekit.ai)
- [ ] `https://debatekit.ai` loads and the browser shows a Cloudflare-issued cert (click the padlock → "Certificate is valid" → issuer is Google Trust Services or Let's Encrypt via CF)
...
- [ ] `dig +short api.debatekit.ai` returns Cloudflare anycast IPs (`104.x.x.x` / `172.x.x.x`)
...
- [ ] `debatekit.ai` — primary, this runbook
...
1. Go to https://dcc.godaddy.com/manage/dns?domainName=debatekit.ai
...
For a permanent removal: delete the zone from CF afterward via **Manage Account → Zones → debatekit.ai → Advanced Actions → Remove Site**.
...
- GoDaddy DNS for debatekit.ai: https://dcc.godaddy.com/manage/dns?domainName=debatekit.ai
- GoDaddy registration search: https://www.godaddy.com/domainsearch/find?domainToCheck=debatekit.ai
- WHOIS check: https://www.whois.com/whois/debatekit.ai
