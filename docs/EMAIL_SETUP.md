...
| Address | Role |
|---|---|
| `noreply@debatekit.ai` | App transactional sender (outbound via SES); also forwards inbound replies to team. |
| `support@debatekit.ai` | Shared support inbox; team replies manually. |
| `hello@mail.debatekit.ai` | Low-volume marketing/newsletter address. Subdomain isolates marketing reputation from transactional. |
| `*@debatekit.ai` (catch-all) | Everything else forwards to the team. |

All inbound forwards to: `ava@deadpixel.ai`, `soheil@deadpixel.ai`.

### Option A — set up via dashboard (recommended for first run)

1. Open <https://dash.cloudflare.com/67bc7b518b92a0c406ac9b8526ddbb6d/debatekit.ai/email/routing>.
...
4. Under **Routing rules**, create:
   - `noreply@debatekit.ai` → forward to both destinations
   - `support@debatekit.ai` → forward to both destinations
   - `hello@mail.debatekit.ai` → forward to both destinations (requires the `mail` subdomain to also have Email Routing enabled; see note below)
   - Catch-all rule → forward to both destinations
5. Send a test from a personal account to confirm.

> Subdomain note: `hello@mail.debatekit.ai` lives on a different DNS name from the apex. Cloudflare Email Routing only handles inbound for zones it manages. Two options: (a) add `mail.debatekit.ai` as its own zone and enable routing there, or (b) drop the marketing subdomain and send/receive marketing from the apex. (a) is preferred for reputation isolation.
...
1. **Verify the domain.**
   Open <https://us-east-1.console.aws.amazon.com/ses/home?region=us-east-1#/verified-identities> → **Create identity** → **Domain** → `debatekit.ai` → enable Easy DKIM (RSA 2048). SES gives you three CNAME records (`<token1>._domainkey.debatekit.ai` → `<token1>.dkim.amazonses.com`, etc.). Add them to the Cloudflare zone (proxy OFF — DKIM lookups need raw CNAMEs).
...
3. **DMARC.**
   Add TXT on `_dmarc.debatekit.ai`:
   ```
   v=DMARC1; p=quarantine; rua=mailto:postmaster@debatekit.ai; ruf=mailto:postmaster@debatekit.ai; fo=1
   ```
...
6. **Create the IAM sender.**
   ...
   ```json
   {
     ...
     "Statement": [{
       ...
       "Resource": "arn:aws:ses:us-east-1:<ACCOUNT_ID>:identity/debatekit.ai"
     }]
   }
   ```
...
|---|---|---|
| `noreply@debatekit.ai` | ava + soheil | yes — auth verification, password reset, account notifications |
| `support@debatekit.ai` | ava + soheil | manual replies from team (via personal client w/ "Send As") |
| `hello@mail.debatekit.ai` | ava + soheil | marketing / newsletters (low volume) |
| `*@debatekit.ai` (catch-all) | ava + soheil | — |

---

## 4. DNS records summary

All records live in the Cloudflare zone for `debatekit.ai`. The script doesn't add these — Cloudflare (for routing) and SES (for DKIM) add them via their dashboards. Listed here for reference.

| Name | Type | Value | Added by | Purpose |
|---|---|---|---|---|
| `debatekit.ai` | MX (prio 10) | `route1.mx.cloudflare.net` | CF Email Routing | Inbound |
| `debatekit.ai` | MX (prio 20) | `route2.mx.cloudflare.net` | CF Email Routing | Inbound |
| `debatekit.ai` | MX (prio 30) | `route3.mx.cloudflare.net` | CF Email Routing | Inbound |
| `debatekit.ai` | TXT | `v=spf1 include:_spf.mx.cloudflare.net include:amazonses.com -all` | merged manually | SPF (both senders) |
...
| `_dmarc` | TXT | `v=DMARC1; p=quarantine; rua=mailto:postmaster@debatekit.ai; fo=1` | manual | DMARC |
...
- [ ] Send a test email from a personal account → `noreply@debatekit.ai`. Confirm it lands in both `ava@deadpixel.ai` and `soheil@deadpixel.ai`.
- [ ] Same for `support@debatekit.ai` and `hello@mail.debatekit.ai`.
- [ ] Send to a made-up address (`random123@debatekit.ai`) to confirm catch-all works.
- [ ] Trigger a password reset from the app — confirm SES delivers the message and the From address is `noreply@debatekit.ai`.
...
| MX records missing on `debatekit.ai` | Email Routing not enabled, or enable failed silently | Open CF dashboard → Email → Routing → **Enable**. If a third-party MX (Google, Fastmail) was there first, CF refuses — remove the conflicting MX records first. |
...
| `hello@mail.debatekit.ai` doesn't receive | Email Routing only handles zones it manages — `mail.debatekit.ai` needs its own zone or the routing rule should target the apex | Add `mail.debatekit.ai` as a separate CF zone and enable Email Routing on it. |
| Mail-tester score 7/10 with "DKIM signed but not aligned" | App is sending from a `From:` that doesn't match the SES-verified domain | Ensure `From:` header uses `noreply@debatekit.ai` exactly (not a subdomain or alias). |
